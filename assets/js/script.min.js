...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function getOS() {
    ...
    if (/android/.test(userAgent)) return 'aos';
  ...
  document.body.addEventListener('click', (event) => {
    // 디맨드 파트너 로고 클릭 (모달)
    const partnerLink = event.target.closest('.partner-logo-link');
    if (partnerLink) {
      event.preventDefault();
      const title = partnerLink.dataset.title;
      const description = partnerLink.dataset.description;
      const url = partnerLink.href;
      openModal(title, description, url);
      return; // 중복 실행 방지
    }

    // 자체 서비스 파트너 로고 클릭 (OS별 링크)
    const servicePartnerLink = event.target.closest('.service-partner-link');
    if (servicePartnerLink) {
      event.preventDefault();
      const os = getOS();
      const url = servicePartnerLink.dataset[`${os}Url`];
      if (url && url !== '#') {
  ...
  if (serviceTabsContainer) {
    serviceTabsContainer.addEventListener('click', (event) => {
      ...
      // 탭 콘텐츠 활성 상태 변경
      const contentContainer = document.querySelector('.service-tab-content');
      contentContainer.querySelector('.active').classList.remove('active');
      contentContainer.querySelector(`#tab-${tabName}`).classList.add('active');
    });
  }
});
